				...
				if (event.messageReply.attachments && event.messageReply.attachments.length > 0) {
					...
					for (const attachment of event.messageReply.attachments) {
						if (attachment.type === "photo") {
							...
							var res = await axios.get(`https://bardapi.easyapi0.repl.co/api/bard?message=${encodeURIComponent(question)}&url=https://newtai-1.easyapi0.repl.co/img/${filename}.jpg&userID=${encodeURIComponent(userId)}&api=ISOYXD`);
							...
							var res = await axios.get(`https://bardapi.easyapi0.repl.co/api/bard?message=${encodeURIComponent(question)} ${encodeURIComponent(result.transcript)}&userID=${encodeURIComponent(userId)}&api=ISOYXD`);
